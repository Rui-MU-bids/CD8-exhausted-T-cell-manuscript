...
#FigS2A-C
p1 <- FeaturePlot(exhaustedT, 'Cd3e', slot = 'data', cols = c('lightblue','steelblue'), min.cutoff = 1)
p2 <- FeaturePlot(exhaustedT, 'Cd8a', slot = 'data', cols = c('lightblue','steelblue'), min.cutoff = 1)
p3 <- FeaturePlot(exhaustedT, 'Pdcd1', slot = 'data',cols = c('lightblue','steelblue'), min.cutoff = 1)
ggsave(filename = "Cd8a.pdf", plot = p2, device = "pdf", width = 8, height = 6) 


table(exhaustedT$humanC_adj, exhaustedT$cancertype)
...
saveRDS(exhaustedT, "mice_exhaustedT.RDS")
...
#FigS2F
subtypes <- c('Ccl3',"Ccl4","Klrd1","Ifng",
              "S100a4","S100a6","S100a13",
              "Il7r","Tcf7","Ccr7","Sell",
              "Ifit1","Ifit2","Ifit3","Oas3","Isg15",
              "Cdca8","Birc5","Cdc45","Mcm3")
...
plot <- DotPlot(exhaustedT, features = Exhaustion, dot.scale = 10, scale = T, cols = c("lightgrey", "#bd7fef"),scale.min = 0, scale.max = 100)+coord_flip()
